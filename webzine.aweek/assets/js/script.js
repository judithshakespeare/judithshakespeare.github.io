$(document).ready(function(){
	...
	$('#about').hover(function( ) {
		$('#about').css('text-shadow', '2px 3px 2px #1a6025');
		$('#firstweek').css('text-shadow', 'none');
		$('.about-wrapper').show();
	},function(){
		$('#about').css('text-shadow', 'none');
		$('#firstweek').css('text-shadow', '2px 3px 2px #1a6025');
		$('.about-wrapper').hide();
	});
	...
	$('.date-wrapper > ul > li > span').click(function(){
		...
		if(index == 37){
			setTimeout(function(){
				$('.diary-wrapper').append('<br><img src="assets/img/clover.png" style="width: 19px">');
			}, 10);
		}else{
		};
	});
	...
	if( /Android|webOS|iPhone|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
		$('.manu-wrapper, .diary-wrapper, .date-wrapper, .container, .sticker-wrapper').hide();
		$('.ifmobile').show();
	};
	...
	Nov12diary[3] 	= '<span><br>오늘 못한 일 중 하나는 완성된 (베타 버전의) 작업을 기록하는 일이다. </span><span class="questions" id="record">저번 학기 작업물인데 아직도 원하는 모양의 기록물을 남기지 못했다.</span> ';
	Nov12diary[4] 	= '<span>사진이든 스캔이든 장비가 가진 한계가 너무 잘 보여서 자주 힘이 빠진다. </span>';
	Nov12diary[5] 	= '<span>그래도 가능한 최선의 방법을 찾아야 한다. 작업물 자체로 보여지는 상황보다 작업이 기록물로 보여지는 상황이 많기 때문에 어쩔 수가 없다. 하하... </span>';
	...
	Nov15diary[1] 	= '<span>식사 후에 엄마와 나는 젓갈을 사러 소래포구로 갔다. </span> 나는 작은 횟집 몇 개가 나란히 있는 거리를 상상했는데, 소래포구에는 어선도 있었고 시장의 규모가 상상 이상으로 컸다. </span>';	
	Nov15diary[2] 	= '<br><img class="diaryImg" src="assets/img/christmas.jpg"><img class="diaryImg" src="assets/img/christmas2.jpg"><span id="christmas">핸드폰을 보다가 어떤 사진에 멈췄다. 올해 처음 본 크리스마스 사진이다. 나는 겨울이 좋다. 겨울의 색이 좋다. 크리스마스의 색도 좋다. </span>';	
	Nov15diary[3] 	= '<span id="snow">눈이 오는 크리스마스를 보고 싶다. 벌써 크리스마스가 한 달 정도밖에 남지 않았다. </span>';	

	var Nov16diary 	= [];
	Nov16diary[0] 	= '<img class="diaryImg" src="assets/img/plant.jpg"><span id="plant">집에 돌아오니 화분에 꽃이 피어있다. 집을 비운 3일간 블라인드가 올라가 있어서 빛을 많이 받은 모양이다. 전에 작업실에서 키우던 화분은 겨울에 추워서 얼어 죽었다. 그 이후로 몇 년간 식물을 키우지 않았는데 두 번째 식물은 다행히 계속 살 거 같다. </span><br>';
	...
	Nov16diary[4] 	= '<br><span>(요즘은 아예 안 하지만) 게임을 할 때도 로그아웃하는 순간이 좋았다. 뭔가를 쏟아내다가 문을 닫아버리는 순간이 좋다. </span>';	
	Nov16diary[5] 	= '<br><span>올해가 얼마 남지 않았다는 생각 때문인지 마음이 급하다. 파주에 오랜만에 왔으니 정리를 해야겠다. </span>';
	...
	Nov17diary[3] 	= '<span>(기록할 만한 명곡은 나세의 최애곡. 자네라는 호칭이 정말 기가 막히게 좋다.) </span>';	
	Nov17diary[4] 	= '<span><br>작업은 생각보다 아주 많이 길어져서, 아침 수업까지 잠깐이라도 잘 시간이 없었다. 분명 2-3시간을 예상했는데 거의 10시간 넘게 마우스를 만지작거렸다. </span>';
	Nov17diary[5] 	= '<span class="questions" id="work">물론 시간이 중요한 것은 아니고 마음에 들어야 하는데 그렇지가 않아서 좀 답답했다.</span> ';
	...
	Nov18diary[2] 	= '<span>시간이 지나서 그 원인을 찾았는데, 드라이기 바람이 너무 강해서 쓰러지는 거였다. 이유를 발견하고 나서는 그냥 장난감을 눕혀놨다. </span>';	
	Nov18diary[3] 	= '<br><span class="questions" id="name">새로 할 작업을 위한 이름을 생각해봤다. 전에 생각해둔 단어인데 지금 꺼내면 되겠다는 생각이 들었다.</span> ';	
	Nov18diary[4] 	= '<span class="questions" id="newWork">아직 정리 못 한 작업들이 많은데 새 작업을 하는 게 괜찮을지 모르겠다.</span> ';	
	Nov18diary[5] 	= '<br><span>이제 꽤 추운데 눈은 언제 올까. 왠지 올해 눈이 엄청 많이 올 거 같다. 파주의 겨울이 어떨지 궁금하다. 일단 겨울에 가까워지니 <span id="star">별</span>이 잘 보여서 좋다. </span>';
	...
	$(document).on('click', '#star', function() {
		...
		$('.date-wrapper, .title, .draggable, .resize, .toyImgResize, .fogVideoResize, .diaryImg').css('visibility', 'hidden');
	...
	$(document).on('click', '#blackStar', function() {
		...
		$('.date-wrapper, .title, .draggable, .resize, .toyImgResize, .fogVideoResize, .diaryImg').css('visibility', 'visible');
	...
	$(document).on('click', '#exhibitionmap', function() {
		...
		var top = $('#exhibitionmap').offset().top;
	...
	$(document).on('click', '#diary', function() {
		...
		window.open("sogam.html", "sogam", "width=250px,height=250px,top=100px,left=200px,scrollbars=yes");
	});
